...
import com.bluemoon.app.dao.statistic.BaoCaoDAO;
...
public class BaoCaoController {
     ...
     * @return List<CongNo>
     */
    public List<Invoice> getChiTietBaoCao(int thang, int nam) {
        try {
    ...
    public boolean xuatBaoCaoTamTruTamVang(File file) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            List<ResidencyRecord> listTamTru = tttvDAO.getByLoaiHinh("TamTru");

            int rowIdx = 1;
            for (ResidencyRecord item : listTamTru) {
                ...
                row.createCell(4)
                        .setCellValue(item.getEndDate() != null ? item.getEndDate().toString() : "Vô thời hạn");
            ...
            List<ResidencyRecord> listTamVang = tttvDAO.getByLoaiHinh("TamVang");

            rowIdx = 1;
            for (ResidencyRecord item : listTamVang) {
