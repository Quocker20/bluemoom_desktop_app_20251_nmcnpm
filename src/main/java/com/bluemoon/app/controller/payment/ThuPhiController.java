...
import com.bluemoon.app.dao.payment.CongNoDAO;
import com.bluemoon.app.dao.payment.GiaoDichDAO;
import com.bluemoon.app.dao.payment.KhoanPhiDAO;
...
public class ThuPhiController {
     ...
     */
    public List<Invoice> getDanhSachCongNo(int thang, int nam, String keyword) {
        try {
     ...
     */
    public List<Fee> getAllKhoanPhi() {
        try {
     ...
     * @return List<KhoanPhi>
     */
    public List<Fee> getListKhoanPhi(String keyword) {
        try {
     ...
     */
    public boolean updateKhoanPhi(Fee kp) {
        if (kp.getUnitPrice() < 0) {
            return false;
        }
        try {
     ...
     */
    public void tinhPhiTuDongChoKhoanPhi(int thang, int nam, Fee kp) {
        if (kp.getType() != AppConstants.PHI_BAT_BUOC) {
            return;
        }

        logger.log(Level.INFO, "[ThuPhiController] Bat dau tinh phi tu dong cho khoan phi: {0}", kp.getName());

        List<Household> listHoKhau;
        try {
        ...
        try {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                for (Household hk : listHoKhau) {
                    double soTien = 0;
                    if ("m2".equalsIgnoreCase(kp.getUnit())) {
                        soTien = kp.getUnitPrice() * hk.getArea();
                    } else {
                        soTien = kp.getUnitPrice();
                    }

                    pstmt.setInt(1, hk.getId());
                    pstmt.setInt(2, kp.getId());
    ...
    public boolean thanhToan(int maCongNo, double soTienThu, String nguoiNop, String ghiChu) {
        ...
        try {
            ...
            Invoice cn = congNoDAO.getById(maCongNo);
            if (cn == null) {
                conn.rollback();
                return false;
            }
     ...
     */
    public Household getHoKhauBySoPhong(String soPhong) {
        try {
     ...
     */
    public String themCongNoDonLe(String soCanHo, Fee kp, int thang, int nam) {
        try {
            Household hk = hoKhauDAO.getBySoCanHo(soCanHo);
            if (hk == null) {
                return "Không tìm thấy căn hộ số: " + soCanHo;
            }

            if (congNoDAO.checkExist(hk.getId(), kp.getId(), thang, nam)) {
                return "Hộ này đã có công nợ '" + kp.getName() + "' trong tháng " + thang + "/" + nam;
            }
     ...
     */
    public Invoice getCongNoById(int id) {
        if (id <= 0) {
            return null;
        }
        try {
    ...
    public String chotSoPhiGuiXe(int thang, int nam) {
        try {
            List<Fee> listPhi = khoanPhiDAO.getAll(); // Lấy tất cả khoản phí đang active
            int countOto = 0;
            int countXeMay = 0;

            for (Fee kp : listPhi) {
                String donVi = kp.getUnit().trim(); // Xóa khoảng trắng thừa cho chắc chắn

                // 1. Xử lý Ô tô
                if ("Phương tiện (Ô tô)".equalsIgnoreCase(donVi)) {
                    int rows = congNoDAO.tinhPhiPhuongTien(thang, nam, kp.getId(), kp.getUnitPrice(), 1);
                    countOto += rows;
                } 
                // 2. Xử lý Xe máy / Xe đạp
                else if ("Phương tiện (Xe đạp/Xe máy)".equalsIgnoreCase(donVi)) {
                    int rows = congNoDAO.tinhPhiPhuongTien(thang, nam, kp.getId(), kp.getUnitPrice(), 2);
                    countXeMay += rows;
                }
            }
