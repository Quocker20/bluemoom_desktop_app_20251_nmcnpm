...
import com.bluemoon.app.dao.dancu.CanHoDAO;
...
public class ThemHoKhauDialog extends JDialog {
    ...
    private JComboBox<CanHo> cbCanHo; 
    ...
    private void initComponents() {
        ...
        cbCanHo.addActionListener(e -> {
            CanHo selected = (CanHo) cbCanHo.getSelectedItem();
            if (selected != null) {
                txtDienTich.setText(String.valueOf(selected.getDienTich()));
            } else {
                txtDienTich.setText("");
            }
        });
    ...
    private void loadDanhSachPhongTrong() {
        try {
            List<CanHo> list = canHoDAO.getDanhSachPhongTrong();
            DefaultComboBoxModel<CanHo> model = new DefaultComboBoxModel<>();
            for (CanHo ch : list) {
                model.addElement(ch);
            }
    ...
    private void btnSaveActionPerformed() {
        CanHo selectedRoom = (CanHo) cbCanHo.getSelectedItem();
        ...
        try {
            ...
            chuHo.setHoTen(txtTenChuHo.getText().trim());
            chuHo.setCccd(txtCCCD.getText().trim());
            chuHo.setNgaySinh(ngaySinh);
            chuHo.setGioiTinh((String) cbGioiTinh.getSelectedItem());
            chuHo.setQuanHe(AppConstants.QH_CHU_HO);
