package com.bluemoon.app.view.guixe;
...
import com.bluemoon.app.controller.guixe.PhuongTienController;
...
public class ThemPhuongTienDialog extends JDialog {
    ...
    private JComboBox<HoKhau> cbHoKhau;
    ...
    private PhuongTien currentPhuongTien = null;
    ...
    private void initComponents() {
        ...
        cbHoKhau.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                ...
                if (value instanceof HoKhau) {
                    HoKhau hk = (HoKhau) value;
                    setText("Phòng " + hk.getSoCanHo() + " - " + hk.getTenChuHo());
                }
                return this;
            }
        });
    ...
    private void loadDanhSachHoDan() {
        List<HoKhau> list = hkController.getAll();
        DefaultComboBoxModel<HoKhau> model = new DefaultComboBoxModel<>();
        for (HoKhau hk : list) {
            model.addElement(hk);
        }
    ...
    public void setEditData(PhuongTien pt) {
        ...
        cbLoaiXe.setSelectedIndex(pt.getLoaiXe() == 1 ? 0 : 1);
        
        // Tìm và chọn đúng hộ khẩu trong combobox
        DefaultComboBoxModel<HoKhau> model = (DefaultComboBoxModel<HoKhau>) cbHoKhau.getModel();
        for (int i = 0; i < model.getSize(); i++) {
            HoKhau hk = model.getElementAt(i);
            if (hk.getMaHo() == pt.getMaHo()) {
    ...
    private void handleSave() {
        ...
        if (isEditMode) {
            // --- LOGIC SỬA ---
            String bienSoCu = currentPhuongTien.getBienSo();
            ...
            // Cập nhật object
            currentPhuongTien.setBienSo(bienSoMoi);
            ...
            HoKhau selectedHk = (HoKhau) cbHoKhau.getSelectedItem();
            if (selectedHk == null) {
            ...
            pt.setMaHo(selectedHk.getMaHo());
            pt.setBienSo(bienSoMoi);
            pt.setLoaiXe(loaiXeValue);
            pt.setTrangThai(1);
